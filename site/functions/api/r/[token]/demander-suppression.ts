...
export const onRequestPost: PagesFunction<Env, "token"> = async (context) => {
  ...
  await fetch(`${supabaseUrl}/rest/v1/pro_avis?id=eq.${payload.avis_id}`, {
    ...
    body: JSON.stringify({
      ...
      moderation_reason: "suppression_demandee_par_artisan",
    }),
  });
    ...
    try {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          subject: `[Suppression demandee] Avis sur ${avis.pros?.nom_entreprise || "fiche pro"}`,
          html: `<p>L'artisan demande la suppression de l'avis suivant sur sa fiche.</p>
...
<p>Decision :</p>
<p><a href="${adminBaseUrl}&action=publier">Maintenir l'avis</a>
&nbsp;|&nbsp;
<a href="${adminBaseUrl}&action=rejeter">Supprimer l'avis</a></p>`,
        }),
      });
    } catch (e) {
      console.error("Resend admin suppression failed:", e);
    }
  }
  ...
  return new Response(null, {
    status: 303,
    headers: {
      Location: `/r/${encodeURIComponent(tokenStr)}/?success=suppression-demandee`,
    },
  });
};
