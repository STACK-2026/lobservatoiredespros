...
<title>Donner mon avis sur ${escapeHtml(nom)} - L'Observatoire des Pros</title>
...
<header class="top">
  <div class="wrap">
    ...
    <a class="back" href="/pro/${escapeHtml(slug)}/">&larr; Retour a la fiche</a>
  </div>
</header>

<main>
  <span class="eyebrow">Donner mon avis</span>
  <h1>Partagez votre experience sur ${escapeHtml(nom)}.</h1>
  ...
  <form method="POST" action="/api/avis/submit" enctype="application/x-www-form-urlencoded">
    <input type="hidden" name="pro_slug" value="${escapeHtml(slug)}">

    <div class="field">
      <label for="pseudo">Pseudo public (2 a 30 caracteres)</label>
      <input type="text" id="pseudo" name="pseudo" required minlength="2" maxlength="30" placeholder="Votre pseudo affiche" autocomplete="nickname">
    </div>
    ...
    <div class="field">
      ...
      <div class="verdict-options">
        <label><input type="radio" name="verdict" value="oui" required><span>Oui, je recommande</span></label>
        <label><input type="radio" name="verdict" value="mitige"><span>Mitige</span></label>
        <label><input type="radio" name="verdict" value="non"><span>Non, je deconseille</span></label>
      </div>
    </div>

    <div class="field">
      <label for="texte">Votre recit (100 a 5000 caracteres)</label>
      <textarea id="texte" name="texte" required minlength="100" maxlength="5000" placeholder="Decrivez votre experience de maniere factuelle..."></textarea>
